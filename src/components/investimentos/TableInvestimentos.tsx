      ...
      <table className="min-w-full bg-[#077a7d33] border border-[rgba(245,238,221,0.6)] rounded-md shadow-sm text-[#f5eedd] text-sm">
        <thead className="bg-[rgba(122,226,207,0.5)] text-[#f5f5f5] text-sm">
          <tr>
            ...
            <th className="px-4 py-2 text-center hidden lg:table-cell">Instituição</th>
        ...
        <tbody className="text-sm font-medium text-white">
          {investimentos.map((inv) => (
            <tr key={inv.id} className="border-t border-white/20 text-center">
              <td className="px-4 py-3">{inv.nomeAplicacao}</td>
              <td className="px-4 py-3">{inv.tipoInvestimento}</td>
              <td className="px-4 py-3">R$ {inv.valorAplicacao.toFixed(2)}</td>
              <td className="px-4 py-3 hidden lg:table-cell">{inv.instituicao}</td>
              <td className="px-4 py-3 hidden lg:table-cell">
                {inv.dataRealizacao ? new Date(inv.dataRealizacao).toLocaleDateString() : 'N/A'}
              </td>
              <td className="px-4 py-3">
                {inv.dataVencimento ? new Date(inv.dataVencimento).toLocaleDateString() : 'N/A'}
              </td>
              <td className="px-4 py-3">
