...
export default function CardGrupo({
  ...
  return (
    <div className="w-full sm:w-[90%] md:w-[80%] lg:w-[70%] xl:w-[30%] bg-[#0c5270] border border-gray-300 rounded-md shadow-sm p-4 h-[400px] overflow-y-auto flex-shrink-0">
      <section aria-labelledby={`grupo-${tipo}`} className="min-h-0">
        <h3 id={`grupo-${tipo}`} className="text-lg font-semibold text-white mb-2">
          {titulo}
        </h3>
        <hr className="border-gray-400 mb-4" />

        {itens.length === 0 ? (
          <p className="text-white text-sm text-center">Nenhum item encontrado.</p>
        ) : (
